...
private:
    ...
    void updateWordCount() {
        auto count = [](const QString& s){
            return s.remove(QRegExp("\\s")).size();
        };
        int curr = count(editContent->toPlainText());
        currentWord->setText(QString("当前章节：%1 字").arg(curr));

        int total = 0;
        for(auto& ch : m_chapters) total += count(ch.content);
